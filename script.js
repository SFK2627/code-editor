...
const adminImportShortcutBtn = document.getElementById('adminImportShortcutBtn');
...
function focusRubricImageImport() {
  ...
  const card = document.querySelector('.rubric-image-import-card');
  if (card) {
    card.scrollIntoView({ behavior: 'smooth', block: 'start' });
    card.classList.add('rubric-import-pulse');
    window.setTimeout(() => card.classList.remove('rubric-import-pulse'), 1200);
  }
}
...
if (adminImportShortcutBtn) adminImportShortcutBtn.addEventListener('click', focusRubricImageImport);
