...
/* USER-DIRECTED FIX STEP 86
   Safe phone Full Editor tools panel.
   - No DOM moving.
   - No homepage/session load changes.
   - Phone full editor only.
   - Keeps Show panel closed when entering full editor.
   - Renames + File to + Page only while phone full editor is active. */
(() => {
  const root = document.documentElement;
  const body = document.body;
  ...
  function syncShowButtonLabel() {
    ...
    toggle.innerHTML = `<span></span><strong>${label}</strong>`;
    toggle.setAttribute('aria-expanded', String(open));
    toggle.setAttribute('aria-label', open ? 'Hide full editor tools' : 'Show full editor tools');
    toggle.title = open ? 'Hide full editor tools' : 'Show full editor tools';
  }

  function syncPageButtonText(active) {
    const addButton = document.getElementById('addHtmlPageBtn');
    if (!addButton) return;
    if (!addButton.dataset.step86DefaultText) addButton.dataset.step86DefaultText = addButton.textContent || '+ File';
    addButton.textContent = active ? '+ Page' : addButton.dataset.step86DefaultText;
  }
  ...
  function syncPhoneFullEditorTools() {
    ...
    // Start every phone full-editor session clean. Do not leave the Show drawer open
    // from normal mobile mode or from a previous full-editor session.
    if (active && !wasActive) {
  ...
  document.addEventListener('click', event => {
    if (event.target?.closest?.('#mobileEditorToolsToggle, #fullEditorBtn, #exitEditorStickyBtn, #exitEditorBtn')) {
      window.setTimeout(syncPhoneFullEditorTools, 0);
      window.setTimeout(syncPhoneFullEditorTools, 80);
    }
  }, true);
  ...
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', syncPhoneFullEditorTools, { once: true });
  else syncPhoneFullEditorTools();

  window.MCS_STEP86_PHONE_FULL_EDITOR_TOOLS_STATUS = () => ({
    phoneUi: isPhoneUi(),
    active: isPhoneFullEditorActive(),
    toolsOpen: body.classList.contains('mobile-editor-tools-open'),
    showButtonFound: Boolean(document.getElementById('mobileEditorToolsToggle')),
    suggestionToggleFound: Boolean(document.getElementById('mobileSuggestionToggle'))
  });
})();
