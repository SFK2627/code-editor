...
# Rubric Image Import Setup

The Teacher/Admin page now has **Upload Rubric Picture**.

This feature needs the secure Firebase Function named `rubricImageImport` because the website is on GitHub Pages and API keys must not be exposed in frontend files.

## Steps

1. Make sure the functions folder is installed:
...
2. Set your OpenAI API key as a Firebase secret:
...
4. After deploy, copy the HTTPS URL for `rubricImageImport`.

5. Open `firebase-config.js` and paste it here:

```js
window.MCS_RUBRIC_IMAGE_ENDPOINT = 'https://YOUR_FUNCTION_URL/rubricImageImport';
```

6. Upload the updated frontend files to GitHub again.

## Important

- Teacher must be logged in through Firebase Authentication before using image import.
- The function checks the Firebase login token before reading the rubric image.
- The imported rubric is not automatically saved. Review it first, then click **Save Activity**.
